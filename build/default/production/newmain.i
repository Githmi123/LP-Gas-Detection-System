...
unsigned int readGasSensorPot(void) {
    ...
    ADCON0bits.GO = 1;
    while (ADCON0bits.GO);
    unsigned int sensorValue = ((ADRESH << 8) + ADRESL);
    return sensorValue;
}
...
{
    float rs = (((1023.0 * 20) / sensorValue) - 20);
    float rs_r0_ratio = rs / 9.83;
    float num = ((log10f(rs_r0_ratio) - curve[1]) / curve[2]) + curve[0];
    unsigned int ppm = 1;
    if (num > 0) {
...
void displayGasConcentration(unsigned int adcValue) {
    ...
    sprintf(concentrationStr, "%u PPM", ppm);
    lcd_string(concentrationStr, strlen(concentrationStr));
}
...
void setServoAngle(int angle) {
    ...
    unsigned int pwmValue = (unsigned int)(duty_cycle * (20000000 / (4.0 * 16.0 * 50.0)) - 1.0);
...
unsigned int getWeightFromSensor(void) {
    ...
    unsigned int weight = totalWeight / 4;
    return weight;
}
